...
[IMPORTANT]
To generate a self signed certificate now you must specify the hostname in your command, the property 'SAN=DNS:0.0.0.0,IP:127.0.0.1' is key in the example below

[source, code, linenums]
----
keytool -genkeypair -keystore keystore.jks   -dname "CN=0.0.0.0, OU=Unknown, O=Unknown, L=Unknown, ST=Unknown, C=Unknown"  -keypass password  -storepass password  -keyalg DSA  -sigalg SHA1withDSA  -keysize 1024  -alias mule  -ext SAN=DNS:0.0.0.0,IP:127.0.0.1 -validity 9999
----
