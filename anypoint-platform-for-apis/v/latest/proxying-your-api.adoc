...
* *Automatic On-premises deploy*: Valid permissions for both the Runtime Manager and the Anypoint Platform for APIs of your organization. Also, your on-premises server must be registered with the Anypoint Platform (link:/runtime-manager/managing-servers#add-a-server[Managing Servers]).  +
