...
[NOTE]
Note that high performance is implemented differently on link:/runtime-manager/cloudhub[CloudHub], so this section applies only for link:/runtime-manager/deploying-to-your-own-servers[on-premises deployments].
