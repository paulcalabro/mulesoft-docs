...
Applications deployed to CloudHub can make use of link:/runtime-manager/link:/runtime-manager/insight[[CloudHub Insight], applications deployed to local servers can link:/runtime-manager/sending-data-from-arm-to-external-monitoring-software[send data to external tools, such as Splunk or ELK].
