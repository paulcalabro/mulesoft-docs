...
[NOTE]
Insights is a feature that is currently only available for applications deployed to CloudHub workers. It is not available for applications that you deploy via the Runtime Manager to local servers on premises or in the cloud. See link:/cloudhub-and-mule[CloudHub and Mule] for more details.
