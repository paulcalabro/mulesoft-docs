...
CloudHub Fabric requires a CloudHub Enterprise or Partner account type. This document assumes that you have an account type that allows you to use this feature, and that you are familiar with link:/runtime-manager/deploying-a-cloudhub-application[deploying applications using the Runtime Manager console]. 
