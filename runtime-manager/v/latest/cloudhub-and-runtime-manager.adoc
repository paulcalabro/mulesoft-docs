...
Applications deployed to CloudHub can make use of link:/runtime-manager/cloudhub-insight[CloudHub Insight], applications deployed to external servers can link:/runtime-manager/sending-data-from-arm-to-external-monitoring-software[send data to external tools, such as Splunk or ELK].
